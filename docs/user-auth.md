...
| 功能 | 鉴权 | 说明 |
|------|------|------|
| Web Dashboard（收件箱、发件箱、规则等） | Session Cookie | 须登录；默认演示账号 `guest` / `guest` |
...
同时，若尚无 `guest` 用户，会自动创建演示账号 **guest / guest**（日发信配额 50）。可直接访问 <SiteLink to="/login">/login</SiteLink> 体验，或在 <SiteLink to="/dashboard/api-keys">/dashboard/api-keys</SiteLink> 创建 API Token。

打开 `https://你的域名/{ADMIN_PATH}` → **用户** 标签创建更多用户并设置日发信配额；生产环境可禁用或删除 `guest`。
