  ...
  <ol>
    ...
    <li>点击 <strong>New repository secret</strong> 并添加以下七个密钥：
      <ul>
        ...
        <li><code>MAILCHANNELS_API_KEY</code>: MailChannels Email API Key（scope 选 <code>api</code>），用于 <code>/api/send</code> 发信。</li>
      </ul>
    </li>
...
`/api/send` 使用 [MailChannels Email API](https://www.mailchannels.com/) 发送邮件。自 2024 年起需 **API Key + Domain Lockdown DNS**，不再支持仅 `cfid=` 的无密钥发信。
